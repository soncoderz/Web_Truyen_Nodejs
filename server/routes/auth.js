...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user) {
      return res.status(401).json(buildMessage("Error: Invalid username or password!"));
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingUsername) {
      return res.status(400).json(buildMessage("Error: Username is already taken!"));
    }

    const existingEmail = await User.exists({ email: req.body.email });
    if (existingEmail) {
      return res.status(400).json(buildMessage("Error: Email is already in use!"));
    }
    ...
    res.json(buildMessage("User registered successfully!"));
  }),
);
  ...
  asyncHandler(async (req, res) => {
    try {
      if (!googleClient || !env.googleClientId) {
        return res
          .status(400)
          .json(buildMessage("Error: Google authentication failed! Missing Google client ID."));
      }
      ...
      if (!payload) {
        return res.status(400).json(buildMessage("Error: Invalid Google token!"));
      }
      ...
      return res
        .status(400)
        .json(buildMessage(`Error: Google authentication failed! ${error.message}`));
    }
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user) {
      return res.status(400).json(buildMessage("Error: Email not found!"));
    }
    ...
    if (!sent) {
      return res.status(500).json(buildMessage("Error: Could not send email."));
    }

    res.json(buildMessage("Password reset email sent successfully."));
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user) {
      return res.status(400).json(buildMessage("Error: Invalid Token!"));
    }

    if (user.resetTokenExpiry && user.resetTokenExpiry.getTime() < Date.now()) {
      return res.status(400).json(buildMessage("Error: Token expired!"));
    }
    ...
    res.json(buildMessage("Password reset successfully!"));
  }),
);
