...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!canViewStory(story, req.user)) {
      throw httpError(403, "Lỗi: Bạn không có quyền bookmark truyện này.");
    }
    ...
    if (normalizedPageIndex !== null && normalizedParagraphIndex !== null) {
      throw httpError(400, "Lỗi: Bookmark chỉ có thể trỏ đến một trang hoặc một đoạn.");
    }
    ...
    if (normalizedTextSnippet !== null && normalizedParagraphIndex === null) {
      throw httpError(400, "Lỗi: Cần có chỉ số đoạn khi lưu trích đoạn văn bản.");
    }

    if (story.type === "MANGA") {
      if (normalizedParagraphIndex !== null || normalizedTextSnippet !== null) {
        throw httpError(400, "Lỗi: Bookmark manga phải trỏ đến một trang.");
      }
    } else if (normalizedPageIndex !== null) {
      throw httpError(400, "Lỗi: Bookmark novel phải trỏ đến một đoạn.");
    }
    ...
    if (normalizedChapterId) {
      ...
      if (!isApprovedStatus(chapterDocument.approvalStatus) && !canViewStory(story, req.user)) {
        throw httpError(403, "Lỗi: Bạn không có quyền bookmark chương này.");
      }
      ...
      if (normalizedParagraphIndex !== null) {
        ...
        if (normalizedNote === null) {
          normalizedNote = `Đoạn ${normalizedParagraphIndex + 1}`;
        }
      }
    }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (String(bookmark.userId) !== String(user.id)) {
      throw httpError(403, "Lỗi: Bạn không có quyền xóa bookmark này.");
    }
