...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    const [totalStories, totalUsers, totalChapters, totalComments, pendingReports, pendingStories, pendingChapters, recentStories] =
      await Promise.all([
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    const [newUsersToday, newUsersThisWeek, newUsersThisMonth, newStoriesThisWeek, newStoriesThisMonth, newChaptersThisWeek, newChaptersThisMonth] =
      await Promise.all([
...
router.get(
  "/stats/distribution",
  asyncHandler(async (_req, res) => {
    const [mangaCount, novelCount, ongoingCount, completedCount, droppedCount, allUsers, roles] =
      await Promise.all([
    ...
    res.json({
      ...
      byStatus: [
        { name: "Dang ra", value: ongoingCount },
        { name: "Da hoan thanh", value: completedCount },
        { name: "Da drop", value: droppedCount },
      ],
      byRole: [
