...
async function sendNewChapterNotifications(story, chapter) {
  ...
  await Notification.insertMany(
    followers.map((user) => ({
      ...
      message: `Chương mới: ${chapter.title}`,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    if (!isObjectId(req.params.storyId)) {
      throw httpError(400, "Lỗi: Mã truyện không hợp lệ.");
    }
    ...
    if (!canManageStory(serializeDoc(story), req.user)) {
      throw httpError(403, "Lỗi: Bạn không có quyền xem các chương này.");
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!isObjectId(req.params.id)) {
      return optional
        ? res.json(null)
        : res.status(400).json(buildMessage("Lỗi: Mã chương không hợp lệ!"));
    }
    const chapter = await Chapter.findById(req.params.id);
    if (!chapter) {
      return optional
        ? res.json(null)
        : res.status(400).json(buildMessage("L?i: Kh�ng t�m th?y chuong!"));
    }
    ...
    if (!story) {
      return optional
        ? res.json(null)
        : res.status(400).json(buildMessage("L?i: Kh�ng t�m th?y truy?n!"));
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!canManageStory(serializeDoc(story), req.user)) {
      throw httpError(403, "Lỗi: Bạn không có quyền cập nhật chương này.");
    }
