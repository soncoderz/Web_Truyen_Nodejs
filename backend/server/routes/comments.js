...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const comments = await Comment.find({ storyId: req.params.storyId })
      .sort({ createdAt: -1 })
      .lean();
    res.json(comments.map(serializeDoc));
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!hasText(req.body.content) && !hasText(req.body.gifUrl)) {
      throw httpError(400, "Error: Comment content or GIF is required!");
    }

    if (req.body.gifSize && Number(req.body.gifSize) > 2 * 1024 * 1024) {
      throw httpError(400, "Error: GIF size must be <= 2MB!");
    }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!comment) {
      throw httpError(400, "Error: Comment not found!");
    }

    const isAdmin = req.user.roles?.includes("ROLE_ADMIN");
    if (!isAdmin && String(comment.userId) !== String(user.id)) {
      throw httpError(400, "Error: Not authorized!");
    }

    await comment.deleteOne();
    res.json(buildMessage("Comment deleted successfully!"));
  }),
);
